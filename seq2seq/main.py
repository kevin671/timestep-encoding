...
parser.add_argument("--config-file", default=None, help="json configuration file")
parser.add_argument(
    "--dataset",
    metavar="DATASET",
    default="WMT16_de_en",
    choices=datasets.__all__,
    help="dataset used: " + " | ".join(datasets.__all__) + " (default: WMT16_de_en)",
)
parser.add_argument("--dataset-dir", metavar="DATASET_DIR", help="dataset dir")
parser.add_argument(
    "--data-config",
...
parser.add_argument(
    "--model-config",
    default="{'hidden_size:256','num_layers':2}",
...
parser.add_argument(
    ...
    help="trainer used: " + " | ".join(trainers.__all__) + " (default: Seq2SeqTrainer)",
)
parser.add_argument(
    "--dtype",
    default="float",
    help="type of tensor: " + " | ".join(torch_dtypes.keys()) + " (default: float)",
)
parser.add_argument(
    "-j",
    "--workers",
...
def main(args):
    ...
    trainer_options = dict(
        ...
        save_path=save_path,
        save_info={"tokenizers": train_data.tokenizers, "config": args},
        regime=regime,
    ...
    if args.tensorwatch:
        trainer.set_watcher(
            filename=os.path.abspath(os.path.join(save_path, "tensorwatch.log")),
    ...
    logging.info(
        "Decoder - number of parameters: %d",
