...
from .config import BOS, EOS, LANGUAGE_TOKENS


def average_models(checkpoint_filenames):
    averaged = {}
    scale = 1.0 / len(checkpoint_filenames)
...
class Translator(object):
    ...
    def __init__(
        ...
        if (
            use_moses is None
        ):  # if not set, turn on if training was done with moses pretok
        ...
        self.pack_encoder_inputs = getattr(self.model.encoder, "pack_inputs", False)
    ...
    def translate(self, input_sentences, target_priming=None):
        ...
        if self.pack_encoder_inputs:
            ...
            sorted_idx, src_tok = zip(
                *sorted(enumerate(src_tok), key=lambda x: x[1].numel(), reverse=True)
            )
                ...
                bos = [
                    list(
                        self.target_tok.tokenize(
                            target_priming[i], insert_start=self.insert_target_start
                        )
                    )
                    for i in order
                ]
        ...
        if self.get_attention:
            ...
            preds = [[self.target_tok.idx2word(idx) for idx in p] for p in preds]
            preds = preds[0] if flatten else preds
            src = [[self.src_tok.idx2word(idx) for idx in list(s)] for s in src_tok]
