...
__all__ = ["Seq2SeqTrainer", "MultiSeq2SeqTrainer", "Img2SeqTrainer", "NestedTrainer"]
...
def _chunk_tuple(seq_tuple, num_chunks, duplicates=1, batch_first=True):
    ...
    chunked_length = [l.tolist() for l in torch.tensor(length).chunk(num_chunks)]
    chunks = []
    for chunk, chunk_length in zip(
        seq.chunk(num_chunks, dim=batch_dim), chunked_length
    ):
...
def _batch_max_tokens(src_tuple, target_tuple, max_tokens, batch_first=True, log=True):
    ...
    num_tokens = src.size(0) * src.size(1) + target.size(0) * target.size(1)
...
class TeacherForcing(DecodedInputTargets):
    ...
    def __call__(self, src_pair, target_pair):
        src, inputs, targets = super(TeacherForcing, self).__call__(
...
class Seq2SeqTrainer(object):
    ...
    def __init__(
        ...
        self.results = ResultsLog(results_file, params=save_info.get("config", None))
        self.watcher = None
        self.streams = {}
    ...
    def iterate(
        self, src_tuple_batch, target_tuple_batch, training=True, chunk_batch=1
    ):
        loss_measure = 0
        accuracy_measure = 0
        nll_measure = 0
        ...
        repacked_inputs = []
        for src_tuple, target_tuple in zip(
            _chunk_tuple(
                src_tuple_batch, chunk_batch, self.duplicates, self.batch_first
            ),
            _chunk_tuple(
                target_tuple_batch, chunk_batch, self.duplicates, self.batch_first
            ),
        ):
            ...
            encoded, decoded, target = self.target_forcing(src_tuple, target_tuple)
        ...
        if training:
            if self.grad_clip is not None:
                if isinstance(self.grad_clip, dict):
                    ...
                    if clip_encoder > 0:
                        clip_grad_norm_(self.model.encoder.parameters(), clip_encoder)
                    if clip_decoder > 0:
                        clip_grad_norm_(self.model.decoder.parameters(), clip_decoder)
            ...
            if self.embedding_grad_clip is not None and self.embedding_grad_clip > 0:
                if hasattr(self.model.encoder, "embedder"):
                    clip_grad_norm_(
                        self.model.encoder.embedder.parameters(),
                        self.embedding_grad_clip,
                    )
                if hasattr(self.model.decoder, "embedder"):
                    clip_grad_norm_(
                        self.model.decoder.embedder.parameters(),
                        self.embedding_grad_clip,
                    )
            self.optimizer.step()
    ...
    def _feed_data(
        self, data_loader, num_iterations=None, training=True, chunk_batch=1
    ):
        if training:
        ...
        for i, (src, target) in enumerate(data_loader):
            ...
            try:
                if training:
                    self.epoch += 1.0 / len(data_loader)
                    if self.distributed:
                ...
                loss, nll, acc, num_words = self.iterate(
                    src, target, training=training, chunk_batch=chunk_batch
                )

                if self.local_rank == 0 and i % 100 == 0:
                    wandb.log(
                        {"loss": loss, "perplexity": math.exp(nll), "accuracy": acc}
                    )
                ...
                if training and "out of memory" in str(err):
                    logging.info("WARNING: ran out of memory, skipping batch")
            ...
            if i > 0 or last_iteration:
                if hasattr(self.target_forcing, "keep_prob"):
                    ...
                    self.target_forcing.keep_prob = max(0.0, prob - 1e-5)

                if i % self.print_freq == 0 or last_iteration:
                    if hasattr(self.target_forcing, "keep_prob"):
                        logging.info("keep-prob = %s" % self.target_forcing.keep_prob)
                    logging.info(
                        "{phase} - Epoch: [{0}][{1}/{2}]\t"
                    ...
                    self.stream_meters(meters, prefix="train" if training else "eval")
                    ...
                    if training:
                        self.write_stream(
                            "lr", (self.training_steps, self.optimizer.get_lr()[0])
                        )
    ...
    def run(self, train_loader, val_loader=None):
        for train_results in self.optimize(train_loader):
            ...
            self.results.plot(
                x="training steps", y=plot_accuracy, title="Accuracy", ylabel="accuracy"
            )
            self.results.plot(
                x="training steps", y=plot_loss, title="Loss", ylabel="loss"
            )
            self.results.plot(
                x="epoch", y=plot_perplexity, title="Perplexity", ylabel="perplexity"
            )
            self.results.plot(
                x="epoch", y=plot_accuracy, title="Accuracy", ylabel="accuracy"
            )
            self.results.plot(x="epoch", y=plot_loss, title="Loss", ylabel="loss")
            self.results.save()
    ...
    def save(self, filename=None, identifier=None, is_best=False, save_all=False):
        if (
            self.distributed and torch.distributed.get_rank() > 0
        ...
        if is_best:
            shutil.copyfile(filename, os.path.join(self.save_path, "model_best.pth"))
        if save_all:
            shutil.copyfile(
                filename,
                os.path.join(self.save_path, "checkpoint_epoch_%s.pth" % self.epoch),
            )
    ...
    def iterate(self, src, target, training=True, chunk_batch=1):
        ...
        def pad_copy(x, length):
            if x.size(time_dim) == length:
                ...
                sz = (x.size(0), length) if self.batch_first else (length, x.size(1))
...
class NestedTrainer(Seq2SeqTrainer):
    ...
    def iterate(self, src_tuple, target_tuple, training=True):
        ...
        if self.max_tokens is not None:
            src_tuple, target_tuple = self._batch_max_tokens(src_tuple, target_tuple)
