...
from seq2seq.tools.config import UNK
...
class Seq2Seq(nn.Module):
    ...
    def forward(
        self, input_encoder, input_decoder, encoder_hidden=None, device_ids=None
    ):
        if not isinstance(device_ids, dict):
            device_ids = {"encoder": device_ids, "decoder": device_ids}
        context = self.encode(
            input_encoder, encoder_hidden, device_ids=device_ids.get("encoder", None)
        )
    ...
    def _decode_step(
        ...
        if feed_all_timesteps:
            inputs = [
                torch.tensor(inp, device=device, dtype=torch.long) for inp in input_list
            ]
            inputs = batch_sequences(
                inputs, device=device, batch_first=self.decoder.batch_first
            )[0]
        ...
        decode_inputs = dict(
            get_attention=get_attention, device_ids=device_ids, **args_dict
        )
        if time_multiply > 1:
    ...
    def generate(
        ...
        if autoregressive:
            ...
            raise NotImplementedError("PermutedSequenceGenerator is not implemented")
