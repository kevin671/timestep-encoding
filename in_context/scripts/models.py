...
from nano_gpt import GPT2Config, GPT2Model, TimeDependentGPT2Model
...
def build_model(conf):
    ...
    elif conf.family == "gpt2_time_dependent_loop":
        model = TransformerModelTimeDependentLooped(
            n_dims=conf.n_dims,
            n_positions=conf.n_positions,
            n_embd=conf.n_embd,
            n_layer=conf.n_layer,
...
class TransformerModelLooped(TransformerModel):
    def __init__(
        ...
        super(TransformerModelLooped, self).__init__(n_dims, n_positions, n_embd, n_layer, n_head, pred_type)
...
class TransformerModelTimeDependentLooped(nn.Module):
    def __init__(
        self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4, loop_func="z=f(x+z)", pred_type="regression"
    ):
        ...
        self._read_in = nn.Linear(n_dims, n_embd)
        self._backbone = TimeDependentGPT2Model(self.configuration)
        if self._pred_type == "regression":
            self._read_out = nn.Linear(n_embd, 1)
        elif self._pred_type == "classification":
            self._read_out = nn.Linear(n_embd, MAX_NUM_CLASS)  # NOTE: hard-code
    ...
    def f(self, output, embeds, idx):
        if self.loop_func == "z=f(x+z)":
            f_output = self._backbone(inputs_embeds=output + embeds, idx=idx)  # [B, 2n + 1, d]
        elif self.loop_func == "z=f(x*z)":
            f_output = self._backbone(inputs_embeds=output * embeds, idx=idx)  # [B, 2n + 1, d]
        else:
            raise NotImplementedError
